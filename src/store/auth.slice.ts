...
export const login = (from:AuthForm) => (dispatch :AppDispatch) => auth.login(from).then (user => dispatch(setUser(user)))
export const register = (from:AuthForm) => (dispatch :AppDispatch) => auth.register(from).then (user => dispatch(setUser(user)))
